...
module "vm_k8s_control" {
  source                            = "./modules/vm"
  for_each                          = { for inst in var.vm_k8s_control_instances : inst.name => inst }
  ...
  vm_networks                       = var.vm_k8s_control_networks
...
resource "ansible_host" "k8s_control" {
  for_each  = { for inst in var.vm_k8s_control_instances : inst.name => inst }
  name      = each.value.name
  groups    = ["kube_control_plane","etcd","kube_node"]
  variables = {
    ansible_user = var.ansible_user
...
output "info_control" {
    value = {
      for vm in var.vm_k8s_control_instances: vm.name => vm.ip
    }
}
